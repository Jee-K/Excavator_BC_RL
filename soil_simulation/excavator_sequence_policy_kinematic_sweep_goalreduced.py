...
CHECKPOINT_ROOT_DIR = "./bc_component/outputs/reduced_mto_emlp"
BEHAVIOR_STREAM = "dump_return"  # toggle between "dig_swing" and "dump_return"
...
def validate_behavior_stream(stream: str) -> str:
    ...
    if stream_l not in {"dig_swing", "dump_return"}:
        raise ValueError(
            f"Unsupported BEHAVIOR_STREAM={stream!r}. Expected 'dig_swing' or 'dump_return'."
        )
    return stream_l
...
class SequenceChunkPolicy:
    ...
    The policy expects:
        ...
        then scaled to [-1, 1]
        raw reduced goal in shape (2,) or flattenable to goal_dim values -> each goal component normalized independently
    ...
    def __init__(
        ...
        self.goal_mean = _as_float_tensor(goal_mean, device).reshape(-1)
        self.goal_std = _as_float_tensor(goal_std, device).reshape(-1)
    ...
    def preprocess_goal(self, goal: np.ndarray | Sequence[float] | Tensor) -> Tensor:
        ...
        mean_dim = int(self.goal_mean.shape[0])
        if goal_t.numel() != mean_dim:
            raise ValueError(f"Expected goal to contain {mean_dim} values, got shape {tuple(goal_t.shape)}")

        g = normalize_goal_components(goal_t, goal_mean=self.goal_mean, goal_std=self.goal_std).view(1, -1)
        return g
...
class ExcavatorSequencePolicyKinematicSweep:
    def __init__(self, viewer) -> None:
        ...
        self.behavior_stream = validate_behavior_stream(BEHAVIOR_STREAM)
        self.urdf_path = Path(URDF_RELATIVE_PATH)
        self.checkpoint_dir = checkpoint_dir_for_behavior_stream(CHECKPOINT_ROOT_DIR, self.behavior_stream)
        self.goal = np.asarray(HARDCODED_GOAL_BY_STREAM[self.behavior_stream], dtype=np.float32)
        ...
        self.policy = load_sequence_policy(
            ...
            joint_indices=None,
            clamp_output=True,
            expected_behavior_stream=self.behavior_stream,
        )
        ...
        print(f"Behavior stream: {self.behavior_stream}")
    ...
    def _advance_policy(self) -> None:
        ...
        pred_chunk = np.asarray(
            self.policy.predict_chunk(current_joint_positions, self.goal),
            dtype=np.float32,
        )
