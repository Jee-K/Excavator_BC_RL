...
class ExcavatorMPMExample:
    def __init__(
        ...
        excavator_urdf = "./excavatorURDF/excavator_boxy_locked.urdf"
        ...
        self.add_bucket_mpm_proxy(builder, excavator_urdf)
    ...
    def add_bucket_mpm_proxy(self, builder, urdf_path: str) -> None:
        """Inflate a watertight MPM-only bucket shell from the URDF collision boxes.

        The previous proxy hard-coded smaller panel sizes than the URDF and even used
        a different front-panel yaw, which leaves seams exactly where soil is forced
        during curl and carry. Here the proxy is derived from the URDF collision boxes
        themselves, then inflated conservatively for MPM contact robustness.
        """
        ...
        if bucket_body is None:
            # !!! Since we collapse names and such, this is yet to activate
        ...
        min_wall = max(self.bucket_seal.min_wall_m, self.bucket_seal.min_wall_voxels * self.voxel_size)
        seam_overlap = max(self.bucket_seal.seam_overlap_m, self.bucket_seal.seam_overlap_voxels * self.voxel_size)
        panels = self._load_bucket_proxy_panels_from_urdf(urdf_path)

        if len(panels) >= 3:
            print(f"Loaded {len(panels)} bucket proxy panels from URDF for sealed MPM contact.")
        else:
            print("Warning: URDF bucket proxy parse produced too few panels; falling back to built-in panel set.")
            panels = self._fallback_bucket_proxy_panels()

        for panel_id, (origin, size, yaw) in enumerate(panels):
            ...
            # Give the cutting-edge/front panel a little extra closure because most
            # leakage occurs at the bottom-front seam during curl.
            if panel_id == 2:
                inflated[0] += 2.0 * self.bucket_seal.front_panel_extra_m
                inflated[1] += 2.0 * self.bucket_seal.front_panel_extra_m
    ...
    def select_mpm_substeps(self) -> int:
        ...
        if distance <= self.contact_margin_m:
        ...
        elif distance <= self.near_margin_m:
