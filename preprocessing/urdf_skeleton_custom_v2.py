...
class SideViewIK:
    """Two-stage FK + IK for side-view excavator fitting.

    Stage 1 — analytic, no optimization:
      1. Rotate 3D skeleton by known cabin_yaw (around Z)
      2. Orthographic-project to 2D (Y horizontal, -Z vertical)
      3. Scale = TRUE physical scale (px/m), constant across all frames.
         Estimated once from frame 0 (cabin_yaw ≈ 0) via arm_base ↔ boom_tip.
      4. Translation: shift so turret_center lands on its 2D annotation.

    Stage 2 — IK:
      With the projection fixed, minimize reprojection error of the 5 arm
      keypoints over the 3 arm joint angles.
      If bucket_tip is visible, scoop1 is solved analytically first (1D search)
      and held fixed; only lower_arm and upperToLow are optimized.
    ...
    def estimate_scale(self, kp_2d_ref: np.ndarray, cabin_yaw_ref: float = 0.0) -> float:
        """Estimate TRUE physical scale using arm_base ↔ boom_tip at cabin_yaw ≈ 0.

        Uses arm joints in rest pose (zero angles) to get the URDF 3D distance,
        then compares to the observed 2D pixel distance at the reference frame.
        arm_base and boom_tip are on the arm chain so their 3D distance is
        well-defined and reliable regardless of frame body foreshortening.
        """
        zero_angles  = np.zeros(self.n_joints)
        kp_3d_ref    = self.skeleton.forward_kinematics(zero_angles)
        phys_dist_3d = np.linalg.norm(kp_3d_ref[self.ARM_BASE_IDX] - kp_3d_ref[self.BOOM_TIP_IDX])
        proj_ref     = self._rotate_and_project(kp_3d_ref, cabin_yaw_ref)
        proj_dist    = np.linalg.norm(proj_ref[self.ARM_BASE_IDX] - proj_ref[self.BOOM_TIP_IDX])
        obs_dist     = np.linalg.norm(kp_2d_ref[self.ARM_BASE_IDX] - kp_2d_ref[self.BOOM_TIP_IDX])
        self.fixed_scale = obs_dist / proj_dist if proj_dist > 1e-6 else None
        print(f'  arm_base↔boom_tip physical dist: {phys_dist_3d:.3f} m')
        print(f'  projected dist at ref frame:     {proj_dist:.3f} m-units')
        print(f'  observed pixel dist:             {obs_dist:.1f} px')
    ...
    def _rotate_and_project(self, kp_3d: np.ndarray, cabin_yaw_rad: float) -> np.ndarray:
        """Rotate by cabin_yaw (Z-axis), then orthographic-project → (N,2) unnormalized."""
        R_z = R.from_euler('z', cabin_yaw_rad).as_matrix()
        kp_world = (R_z @ kp_3d.T).T
        facing = self.facing if self.facing is not None else +1
        # compensate cos(θ) sign flip at |θ| > 90°
        effective_facing = facing * np.sign(np.cos(cabin_yaw_rad))
        if effective_facing == 0:
            effective_facing = facing
        return np.stack([effective_facing * kp_world[:, 1], -kp_world[:, 2]], axis=1)
    ...
    def _solve_scoop1_from_angle(self, kp_2d: np.ndarray, cabin_yaw_rad: float,
                                  current_angles: np.ndarray) -> float:
        """1D solve: find scoop1 that matches the stick_tip→bucket_tip angle in 2D.
        Uses current_angles for lower_arm+upperToLow (fixes the frame orientation).
        """
        from scipy.optimize import minimize_scalar
        stick_2d  = kp_2d[1]  # stick_tip
        bucket_2d = kp_2d[0]  # bucket_tip
        obs_vec   = bucket_2d - stick_2d
        obs_angle = np.arctan2(obs_vec[1], obs_vec[0])

        def angle_loss(scoop1):
            angles    = current_angles.copy()
            angles[2] = scoop1
            kp_3d      = self.skeleton.forward_kinematics(angles)
            proj       = self._rotate_and_project(kp_3d, cabin_yaw_rad)
            pred_vec   = proj[0] - proj[1]  # bucket_tip - stick_tip in projected space
            pred_angle = np.arctan2(pred_vec[1], pred_vec[0])
            diff = (obs_angle - pred_angle + np.pi) % (2 * np.pi) - np.pi
            return diff ** 2

        res = minimize_scalar(angle_loss,
                              bounds=(self.joint_lower[2], self.joint_upper[2]),
                              method='bounded')
        return res.x
    ...
    def fit_frame(self, kp_2d: np.ndarray, vis: np.ndarray,
        ...
        # Stage 1: body keypoints don't depend on arm angles → compute once
        kp_3d_any = self.skeleton.forward_kinematics(init_angles)
        scale, tx, ty = self._projection_params(kp_3d_any, cabin_yaw_rad, kp_2d)

        # Stage 2a: if bucket_tip visible, solve scoop1 analytically from
        # the stick_tip → bucket_tip direction, then fix it.
        bucket_tip_vis = vis[0] > 0.5
        if bucket_tip_vis:
            scoop1_fixed = self._solve_scoop1_from_angle(kp_2d, cabin_yaw_rad, init_angles)
            free_joints  = [0, 1]   # lower_arm, upperToLow only
        else:
            scoop1_fixed = None
            free_joints  = [0, 1, 2]
        ...
        def loss(free_angles):
            angles = init_angles.copy()
            for k, j in enumerate(free_joints):
                angles[j] = free_angles[k]
            if scoop1_fixed is not None:
                angles[2] = scoop1_fixed
        ...
        init_free   = init_angles[free_joints]
        bounds_free = [(self.joint_lower[j], self.joint_upper[j]) for j in free_joints]
        result = minimize(loss, init_free, method='L-BFGS-B',
                          bounds=bounds_free, options={'maxiter': 500})

        final_angles = init_angles.copy()
        for k, j in enumerate(free_joints):
            final_angles[j] = result.x[k]
        if scoop1_fixed is not None:
            final_angles[2] = scoop1_fixed
    ...
    def fit_sequence(self, all_kp_2d: np.ndarray, all_vis: np.ndarray,
        ...
        # Auto-detect facing direction from most side-on frame
        if self.facing is None:
            ...
            if verbose:
                print(f'Facing detected: {side}  '
                      f'(frame_front.x={all_kp_2d[ref_t][6,0]:.0f}  '
                      f'frame_rear.x={all_kp_2d[ref_t][7,0]:.0f})')

        if fix_scale and self.fixed_scale is None:
            # Use frame 0 — cabin_yaw ≈ 0 by construction (all yaw relative to frame 0)
            self.estimate_scale(all_kp_2d[0], cabin_yaw_seq[0])
            if verbose:
                print(f'Scale fixed to {self.fixed_scale:.2f} px/m  '
                      f'(estimated from frame 0 using arm_base↔boom_tip, '
                      f'θ={np.degrees(cabin_yaw_seq[0]):.1f}°)')
        ...
        for t in range(T):
            ...
            if verbose and (t % 20 == 0 or t == T - 1):
                print(f'  Frame {t:3d}/{T}: err={res["error"]:.1f}px  '
